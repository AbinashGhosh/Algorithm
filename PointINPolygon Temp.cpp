...
inline bool inConvexPolygon(pii *C,int nc,pii p)
{
    int lo=1,hi=nc-1,mid;
    while(hi-lo>1)
    {
        mid=(lo+hi)>>1;
        if(cross(C[0],C[mid],p)<0)hi=mid;
        else lo=mid;
    }
    if(cross(C[0],C[lo],p)<0)return false;
    if(cross(C[lo],C[hi],p)<0)return false;
    if(cross(C[hi],C[0],p)<0)return false;
    return true;
}
